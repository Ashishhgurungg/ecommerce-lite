        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="font-semibold text-lg text-center">Add your galleries here</h3>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                ...
                <form action="{{ url('/gallery-form') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                        {{-- gallery Title --}}
                        <label for="name" class="block mb-2 font-medium">gallery Title:</label>
                        <input type="text" id="name" name="name"
                               class="w-full p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200"
                               value="{{ old('name') }}" required>

                        {{-- gallery Description --}}
                        <label for="description" class="block mt-4 mb-2 font-medium">gallery Description:</label>
                        <textarea id="description" name="description" required
                                  class="w-full p-2 border border-gray-300 rounded-md focus:ring focus:ring-indigo-200"
                                  rows="3">{{ old('description') }}</textarea>

                        {{-- gallery Image --}}
                        <label for="image" class="block mt-4 mb-2 font-medium">gallery Image:</label>
